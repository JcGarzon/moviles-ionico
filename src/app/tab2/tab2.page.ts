...
export class Tab2Page implements OnInit {
  ...
  private italyId: string | null = null;
  ...
  private async fetchItalyData() {
    this.loading = true;
    this.error = null;

    try {
      // Step 1: Get Italy's ID
      ...
      if (!italy) {
        throw new Error('País Italia no encontrado');
      }
      this.italyId = italy._id;

      // Step 2: Fetch Cities
      const cities = await this.http
        .get<City[]>(`${this.apiUrl}/cities`, { headers: this.getHeaders() })
        .toPromise();
      this.cities = cities?.filter((city) => city.countryId._id === this.italyId) || [];

      // Step 3: Fetch Sites and filter by Italian cities
      ...
      // Step 4: Fetch Dishes and filter by sites in Italy
      ...
      // Step 5: Fetch Famous People and filter by Italian cities
